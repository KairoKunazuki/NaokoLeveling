...
export default new NativeFunction({
    name: '$setStatLv',
    description: 'Set an amount of level to stat info',
    version: '0.0.1',
    brackets: true,
    unwrap: true,
    args: [
        ...
        {
            name: 'levelAmount',
            description: 'The amount of level to be set',
            rest: false,
            type: ArgType.Number,
            required: true
        },
    ...
    async execute(ctx, [target, amount, region, regionId]) {
        ...
        await stat.setLevel(amount, stat.keyStat(region, regionId));
        return Return.success(amount);
    }
})